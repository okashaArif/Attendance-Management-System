...
firebase_admin.initialize_app(cred,{'storageBucket': 'pythonpicture-8f029.appspot.com'})
db= firestore.client()
...
def save_and_show_image1(url, save_path):
  ...
  try:
      ...
      blob.upload_from_filename(full_save_path)
      blob.make_public()
      data1['image_url'] = blob.public_url
      data1['time']=timestamp
      
      print(f"Image saved successfully to: {full_save_path}")
      print(f"Image URL: {data1['image_url']}")
...
async def Execute():
    ...
    try:
        ...
        data1["Present"] = 1 if result == 1 else 0
            
        doc_ref= db.collection('AttendanceTeacher').document()
        doc_ref.set(data1)
    except Exception as e:
        print(f"Error processing request: {e}")
        return {"message": "Error", "error": str(e)}